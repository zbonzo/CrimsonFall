...
import type { PlayerAction, PlayerActionType } from '@/core/types/playerTypes';
...
export interface ActionResult {
  ...
  readonly actionType: 'move' | 'attack' | 'ability' | 'wait' | 'ai_decision';
...
export class GameLoop {
  ...
  private async processPlayerAction(player: Player, action: PlayerAction): Promise<ActionResult> {
    const baseResult = {
      ...
      actionType: action.type as ActionResult['actionType'],
      success: false,
    };
    ...
    try {
      switch (action.type) {
        ...
        default:
          return { ...baseResult, reason: `Unknown action type: ${action.type}` };
      }
    } catch (error) {
      return {
        ...baseResult,
  ...
  private async processMonsterAction(
    ...
    const baseResult = {
      ...
      actionType: 'ai_decision' as ActionResult['actionType'],
      success: false,
    };
    ...
    try {
      switch (decision.type) {
        ...
        default:
          return { ...baseResult, reason: `Unknown AI decision type: ${decision.type}` };
      }
    } catch (error) {
      return {
        ...baseResult,
